...
begin
  ...
  begin
    if (point_distance(points[0].x, points[0].y, points[1].x, points[1].y) < 1) then
    begin
      case i.ClassName of
        'TPen':
        begin
          ...
          begin
            if (point_distance(wx, wy, i.points[j].x,
              i.points[j].y) < 64) then
            begin
              i.selected := not i.Selected;
              break;
            end;
            if (point_distance(wx, wy, i.points[j].x, i.points[j].y)) <
              mindistance then
              mindistance := point_distance(s2w(x, 0).x, s2w(0, y).y,
                i.points[j].x, i.points[j].y);
          end;
        end;
        'TRectangle':
        begin
    ...
    begin
      case i.ClassName of
        ...
        begin
          for j := 0 to high(i.points) do
          begin
            if (point_in_rectangle(i.points[j].x, i.points[j].y,
              points[0].x, points[0].y, points[1].x, points[1].y)) then
            begin
              i.selected := not i.Selected;
              break;
            end;
